...
CMUTrace::format_mac_common(Packet *p, const char *why, int offset)
{
	...
	sprintf(pt_->buffer() + offset,
		...
		((ch->ptype() == PT_MAC) ? (
		  ...
		  (mh->dh_fc.fc_subtype == MAC_Subtype_Beacon) ? "BCN"  :	
		  (mh->dh_fc.fc_subtype == MAC_Subtype_Command_AssoReq) ? "CM1"  :	
		  (mh->dh_fc.fc_subtype == MAC_Subtype_Command_AssoRsp) ? "CM2"  :	
		  (mh->dh_fc.fc_subtype == MAC_Subtype_Command_DAssNtf) ? "CM3"  :	
		  (mh->dh_fc.fc_subtype == MAC_Subtype_Command_DataReq) ? "CM4"  :	
		  (mh->dh_fc.fc_subtype == MAC_Subtype_Command_PIDCNtf) ? "CM5"  :	
		  (mh->dh_fc.fc_subtype == MAC_Subtype_Command_OrphNtf) ? "CM6"  :	
		  (mh->dh_fc.fc_subtype == MAC_Subtype_Command_BconReq) ? "CM7"  :	
		  (mh->dh_fc.fc_subtype == MAC_Subtype_Command_CoorRea) ? "CM8"  :	
		  (mh->dh_fc.fc_subtype == MAC_Subtype_Command_GTSReq) ? "CM9"  :	
	          "UNKN") :
...
void
CMUTrace::format_aspu(Packet *p, int offset)
{
struct hdr_aspu_pkt* ph = HDR_ASPU_PKT(p);
if (pt_->tagged()) {
sprintf(pt_->buffer() + offset, "-aspu:o %d -aspu:s %d -aspu:l %d ",ph->pkt_src(),ph->pkt_seq_num(),ph->pkt_len());
}
else if (newtrace_) {
sprintf(pt_->buffer() + offset, "-P aspu -Po %d -Ps %d -Pl %d ",ph->pkt_src(),ph->pkt_seq_num(),ph->pkt_len());
}
else {
sprintf(pt_->buffer() + offset, "[aspu %d %d %d] ",ph->pkt_src(),ph->pkt_seq_num(),ph->pkt_len());
}
}
